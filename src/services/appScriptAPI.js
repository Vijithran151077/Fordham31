...
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwx9cBz9ITan3RXGUMi1wbZU_bnuaUivogwVKxn7_DsbpxhkY9_TYVXgb4sZZkum96LSA/exec';
...
export const applyFrontendSmartStatus = (shiftData) => {
  ...
  if (shiftData.shiftDate) {
    const today = getLocalDateString(new Date()); // Get today's date in YYYY-MM-DD format
    ...
    if (shiftDate.includes('T') && shiftDate.includes('Z')) {
      ...
      normalizedShiftDate = getLocalDateString(new Date(shiftDate));
...
export const submitTimeSegments = async (payload) => {
  ...
  if (segments.length > 0) {
    // ✅ DST-SAFE: Anchor segments to real Date objects using the shift date.
    // This lets the JavaScript engine handle DST transitions (e.g. the October
    // clock-back night where 01:00 appears twice and the night has 25 hours).
    const baseDate = shiftDate || getLocalDateString(new Date());
    let currentDayOffset = 0;
    // Track absolute end in minutes (not modulo) for correct rollover detection
    // across multi-segment overnight shifts.
    ...
    segments.forEach((seg) => {
      ...
      // Detect day roll-over between segments (original logic)
      ...
      // Build real local Date objects anchored to the correct calendar day
      const startDay = new Date(`${baseDate}T00:00:00`);
      startDay.setDate(startDay.getDate() + segStartDayOffset);

      const endDay = new Date(`${baseDate}T00:00:00`);
      endDay.setDate(endDay.getDate() + segEndDayOffset);

      const startMs = startDay.getTime() + (startMinutes * 60000);
      const endMs = endDay.getTime() + (endMinutes * 60000);

      // DST-aware duration: JS engine adjusts for clock changes automatically
    ...
    // Total duration from DST-aware segment durations
    totalDuration = timeline.reduce((total, seg) => total + seg.duration, 0);

    console.log(`📊 DST-safe recalculated: firstStartTime=${firstStartTime}, lastEndTime=${lastEndTime}, totalDuration=${totalDuration.toFixed(2)}`);
  }
...
export const detectCrossMidnightShift = async (employeeId, currentDate) => {
  try {
    ...
    const previousDayStr = getLocalDateString(previousDay);

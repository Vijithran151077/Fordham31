...
const AdminDashboard = () => {
  ...
  const getSummaryDateRange = (period, customRange = summaryCustomRange) => {
    ...
    switch (period) {
      ...
      case 'week':
        ...
        return { start: getLocalDateString(weekStart), end: todayStr };
      case 'today':
        // Today only
      ...
      if (dateRange.start && dateRange.end && shiftDate) {
        ...
        const normalizeDate = (dateStr) => {
          ...
          if (typeof dateStr === 'object' && dateStr.getTime) {
            normalized = getLocalDateString(dateStr);
            ...
            if (dateStr.match(/^\d{4}-\d{2}-\d{2}$/)) {
              ...
              if (!isNaN(parsed.getTime())) {
                normalized = getLocalDateString(parsed);
              }
            }
          }
          return normalized;
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const shiftsResponse = await getShifts({
        startDate: getLocalDateString(new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)), // Last 7 days
  ...
  const generateScheduleSuggestions = (staff, recentShifts) => {
    ...
    const weekStart = getLocalDateString(nextWeek);
  ...
  const getDateRangeForPeriod = (period, customRange = customDateRange) => {
    ...
    switch (period) {
      ...
      case 'week':
        ...
        return { start: getLocalDateString(weekStart), end: getLocalDateString(weekEnd) };
      case 'month':
      ...
      case 'quarter':
        ...
        return { start: getLocalDateString(quarterStart), end: getLocalDateString(quarterEnd) };
      case 'year':
        const yearStart = new Date(today.getFullYear(), 0, 1);
        const yearEnd = new Date(today.getFullYear(), 11, 31);
        return { start: getLocalDateString(yearStart), end: getLocalDateString(yearEnd) };
      case 'last7days':
        const last7 = new Date(today);
        last7.setDate(today.getDate() - 6);
        return { start: getLocalDateString(last7), end: todayStr };
      case 'last30days':
        const last30 = new Date(today);
        last30.setDate(today.getDate() - 29);
        return { start: getLocalDateString(last30), end: todayStr };
      case 'custom':
        return customRange;
  ...
  const handleViewData = async () => {
    ...
    try {
      ...
      console.log('Today is:', getLocalDateString(new Date()));
